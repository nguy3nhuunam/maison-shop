...
function serializeProduct(product, options = {}) {
  ...
  const variants = Array.isArray(product.variants)
    ? product.variants.map((variant) => ({
        ...
        price: resolveVariantUnitPrice(product, variant),
  ...
  const minPrice = getProductMinPrice(defaultPrice, variants);
  const maxPrice = variants.reduce((highest, variant) => Math.max(highest, Number(variant.price || 0)), minPrice);

  return {
    id: Number(product.id),
    externalSourceId: String(product.externalSourceId || ""),
    name: product.name,
    shortDescription,
...
function getProductMinPrice(defaultPrice, variants = []) {
  return variants.reduce((lowest, variant) => {
    ...
    return lowest > 0 ? Math.min(lowest, variantPrice) : variantPrice;
...
async function normalizeProductInput(product) {
  const externalSourceId = String(product.externalSourceId || "").trim();
  ...
  if (
    variants.some(
      (variant) =>
        variant.price !== null &&
        (!Number.isFinite(variant.price) || Number(variant.price) <= 0),
    )
  ) {
    throw new Error("Variant price must be greater than zero when provided.");
  }
...
export async function updateProduct(id, productInput) {
  ...
  existing.externalSourceId = product.externalSourceId || existing.externalSourceId || "";
